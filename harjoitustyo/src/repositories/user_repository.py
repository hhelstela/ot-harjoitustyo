...
class UserRepository:
    ...
    def add_user(self, username, password):
        usernames_in_db = self.db.execute("SELECT * FROM Users WHERE username=?", [username]).fetchall()
    ...
    def login_to_service(self, username, password):
        username_in_db = self.db.execute("SELECT * FROM Users WHERE username=?", [username]).fetchall()
        try:
